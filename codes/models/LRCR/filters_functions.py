...
def saturation_filter(img, param):
    ...
    enhanced_img = hsv_to_rgb2([img_hsv[:, 0:1, :, :], enhanced_s, img_hsv[:, 2:3, :, :]])
...
def color_filter(img, param, color_curve_range=(0.90, 1.10), curve_steps=4):
    ...
    for i in range(curve_steps):
        total_img += clip(x=img - 1.0 * i / curve_steps, down=0.0, down_value=0.0,
                          top=1.0 / curve_steps, top_value=1.0 / curve_steps) * param[:, :, :, :, i]
