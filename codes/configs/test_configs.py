...
class TestConfigs(BaseConfigs):
	def __init__(self,
	             # model
	             model_type='', model_version=0, basic_path='G:\WUXU\Jaguar',
	             patch_size2=16, num_heads=4, depth=[1,1,1,1],
        filters_param_ch={'Color': None, 'Contrast': 3, 'Saturation': None, 'WB': None},
        filters='ContrastClip',
	             loss_type='l2',
	             # testing
	             train_dataset_name='', cur_epoch=49,
	             batch_size=1, load_model_path='', test_dataset_name='', is_resize=True, is_real_img=True,
	             img_h=256, img_w=256, patch_size=256):
	...
	def initialize(self):
		...
		self.parser.add_argument('--Testing configs', type=str,
		                         default='---------------------------------------- : Begin')
		...
		self.parser.add_argument('--test_dataset_son', type=str, default=test_dataset_son)      # 成对数据集就有两个文件夹
		...
		self.parser.add_argument('--cur_epoch', type=bool, default=self.cur_epoch)  # 当前测试时采用的是第cur_epoch的模型参数
		...
		# saving path
		# -----------------------------------------------------------------------------------------
		path_main = os.path.join(r'G:\WUXU\Jaguar\logs', self.model_name)
		...
		self.parser.add_argument('--Testing configs END', type=str,
		                         default='--------------------------- : Testing configs END')
	...
	def set_dataset(self, dataset_name):
		...
		# ----------------------------------------------------------------------------
		# 测试数据集
		if dataset_name == 'LLE_test_all':
			# 单张图片
			test_dataset_path = 'D:\WuXu\Code\Python_code\Lightlee_enhancement\data\LLE_testImgs\LLE_test_all'
		elif dataset_name == 'Dark_face200':
			# 单张图片
			test_dataset_path = 'G:\WUXU\Datasets\low_light\Dark_face200'

		elif dataset_name == 'dataset2':
			# 成对的测试图片
			test_dataset_path = 'D:\WuXu\Code\Python_code\Lightlee_enhancement\data\dataset_2'
			test_dataset_son = {'ll': 'test_x', 'org': 'test_y'}  # 低照度


		# 成对真实低照度
		# -------------------------------------------
		elif dataset_name == 'LOLdatasetNASA':
			test_dataset_path = 'E:\Dataset_LL\LOLdataset\eval15'
			test_dataset_son = {'ll': 'test_x', 'org': 'test_y'}  # 低照度
		elif dataset_name == 'LOLdataset_eval15':
			test_dataset_path = 'G:\WUXU\Datasets\LOLdataset\eval15'
			test_dataset_son = {'ll': 'low', 'org': 'high'}  # 低照度
		# -------------------------------------------
		...
		elif dataset_name == 'Synthetic_coco_test':
			test_dataset_path = 'E:\Dataset_LL\Synthetic_coco_test'
			test_dataset_son = {'ll': 'low', 'org': 'high'}  # 低照度
		elif dataset_name == 'Synthetic_coco_test_no_noise':
			test_dataset_path = 'E:\Dataset_LL\Synthetic_coco_test_no_noise'
			test_dataset_son = {'ll': 'low', 'org': 'high'}  # 低照度
		elif dataset_name == 'gamaLinear_Noise':
			test_dataset_path = 'E:\Dataset_LL\gamaLinear_Noise'
			test_dataset_son = {'ll': 'gamaLinear_Noise_TestDim512', 'org': 'gamaLinear_Noise_TestRGB512'}  # 低照度
		elif dataset_name == 'SID_test':
			test_dataset_path = 'D:\WuXu\SID_Test'
			test_dataset_son = {'ll': 'test_x', 'org': 'test_y'}  # 低照度
		elif dataset_name == 'SLD_guass_linear1_test':
			test_dataset_path = r'E:\Dataset_LL\Synthetic_Lowlight_Dataset_wx'
			test_dataset_son = {'ll': 'test_low_gaussian_wx_linear_trans1', 'org': 'test_high'}  # 低照度
		elif dataset_name == 'SLD_guass_linear1_test_200':
			test_dataset_path = r'E:\Dataset_LL\Synthetic_Lowlight_Dataset_wx'
			test_dataset_son = {'ll': 'test_low_gaussian_wx_linear_trans1_200', 'org': 'test_high_200'}  # 低照度
		elif dataset_name == 'SLD_guass_linear1_test_200_rename':
			test_dataset_path = r'E:\Dataset_LL\Synthetic_Lowlight_Dataset_wx'
			test_dataset_son = {'ll': 'test_low_gaussian_wx_linear_trans1_200_rename',
			                    'org': 'test_high_200_rename'}  # 低照度
		else:
